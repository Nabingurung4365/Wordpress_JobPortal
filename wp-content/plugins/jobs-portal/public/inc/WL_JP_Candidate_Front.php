...
class WL_JP_Candidate_Front {
	...
	public static function register_candidate_post_type() {
		$labels = array(
			'name'                  => _x( 'Candidates', 'Post Type General Name', WL_JP_DOMAIN ),
			'singular_name'         => _x( 'Candidate', 'Post Type Singular Name', WL_JP_DOMAIN ),
			'menu_name'             => __( 'Candidates', WL_JP_DOMAIN ),
			'name_admin_bar'        => __( 'Candidate', WL_JP_DOMAIN ),
			'archives'              => __( 'Candidate Archives', WL_JP_DOMAIN ),
			'attributes'            => __( 'Candidate Attributes', WL_JP_DOMAIN ),
			'all_items'             => __( 'All Candidates', WL_JP_DOMAIN ),
			'add_new_item'          => __( 'Add New Candidate', WL_JP_DOMAIN ),
			'add_new'               => __( 'Add New', WL_JP_DOMAIN ),
			'new_item'              => __( 'New Candidate', WL_JP_DOMAIN ),
			'edit_item'             => __( 'Edit Candidate', WL_JP_DOMAIN ),
			'update_item'           => __( 'Update Candidate', WL_JP_DOMAIN ),
			'view_item'             => __( 'View Candidate', WL_JP_DOMAIN ),
			'view_items'            => __( 'View Candidates', WL_JP_DOMAIN ),
			'search_items'          => __( 'Search Candidate', WL_JP_DOMAIN ),
			'not_found'             => __( 'Not found', WL_JP_DOMAIN ),
			'not_found_in_trash'    => __( 'Not found in Trash', WL_JP_DOMAIN ),
			'items_list'            => __( 'Candidate list', WL_JP_DOMAIN ),
			'items_list_navigation' => __( 'Candidate list navigation', WL_JP_DOMAIN ),
			'filter_items_list'     => __( 'Filter Candidate list', WL_JP_DOMAIN ),
		);
		$args = array(
			'label'                 => __( 'Candidate', WL_JP_DOMAIN ),
			'labels'                => $labels,
	...
	public static function register_cv() {
		...
		if ( empty( $personal_name ) ) {
			$errors['candidate_personal_name'] = __( 'Please specify your name', WL_JP_DOMAIN );
		}
		...
		update_post_meta( $post_id, 'wljp_candidate_user_id', $user_id );
		...
		wp_send_json_success( array( 'message' => __( 'Thank you for CV registration.', WL_JP_DOMAIN ), 'reload' => true ) );
	}
	...
	public static function update_cv() {
		...
		$user_id = get_current_user_id();
		if ( $candidate = WL_JP_Helper::user_has_cv( $user_id ) ) {
		...
		if ( is_wp_error( $status ) ) {
			wp_send_json_error( __( $status->get_error_message(), WL_JP_DOMAIN ) );
		}
		...
		wp_send_json_success( array( 'message' => __( 'Your CV has been updated.', WL_JP_DOMAIN ), 'reload' => true ) );
	}
	...
	public static function delete_cv() {
		...
		if ( ! $success ) {
  			throw new Exception( __( 'An unexpected error occurred.', WL_JP_DOMAIN ) );
		}

		wp_send_json_success( array( 'message' => __( 'Your CV has been deleted.', WL_JP_DOMAIN ), 'reload' => true ) );
	}
	...
	public static function apply_to_job() {
		...
		try {
			...
			wp_send_json_success( array( 'message' => __( 'You have applied.', WL_JP_DOMAIN ) ) );
